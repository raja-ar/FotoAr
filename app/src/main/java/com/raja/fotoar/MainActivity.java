...
public class MainActivity extends Activity {
	...
	private static final String API_SECRET = "@string/aviary_secret";

	public static final String LOG_TAG = "fotoar-launcher";

	/** apikey is required http://developers.aviary.com/ */

	/** Folder name on the sdcard where the images will be saved **/
	private static final String FOLDER_NAME = "FotoAr";
	...
	protected void onCreate( Bundle savedInstanceState ) {
		...
		setContentView( com.raja.fotoar.R.layout.main );
	...
	//	Toast.makeText( this, "fotoAr: " + getLibraryVersion() + ", sdk: " + SDKUtils.SDK_VERSION, Toast.LENGTH_SHORT ).show();
	...
	public void onContentChanged() {
		...
		mGalleryButton = (Button) findViewById( com.raja.fotoar.R.id.button1 );
		mEditButton = (Button) findViewById( com.raja.fotoar.R.id.button2 );
		mImage = ( (ImageView) findViewById( com.raja.fotoar.R.id.image ) );
		mImageContainer = findViewById( com.raja.fotoar.R.id.image_container );
	}

	@Override
	public boolean onCreateOptionsMenu( Menu menu ) {
		MenuInflater inflater = getMenuInflater();
		inflater.inflate( com.raja.fotoar.R.menu.main_menu, menu );
		return true;
	}
	...
	public boolean onOptionsItemSelected( MenuItem item ) {
		...
		final int id = item.getItemId();
		 ...
		 if ( id == com.raja.fotoar.R.id.view_documentation ) {
...
//			startActivity( intent );
		} else if ( id == com.raja.fotoar.R.id.dev ) {
	...
	protected Dialog onCreateDialog( int id ) {
		...
		// external sdcard is not mounted!
			case EXTERNAL_STORAGE_UNAVAILABLE:
				dialog = new AlertDialog.Builder( this ).setTitle( com.raja.fotoar.R.string.external_storage_na_title )
						.setMessage( com.raja.fotoar.R.string.external_storage_na_message ).create();
				break;
		}
		return dialog;
	}
	...
	private File getNextFileName() {
		if ( mGalleryFolder != null ) {
			if ( mGalleryFolder.exists() ) {
				File file = new File( mGalleryFolder, "fotoar_" + System.currentTimeMillis() + ".jpg" );
				return file;
			}
		}
		return null;
	...
	private class HDAsyncTask extends AsyncTask<Cursor, Integer, String> {
		...
		private void saveExif( ExifInterfaceExtended originalExif, String filename ) {
			...
			if ( null != newExif && null != originalExif ) {
				...
				newExif.setAttribute( ExifInterfaceExtended.TAG_EXIF_SOFTWARE, "FotoAr " + SDKUtils.SDK_VERSION );
				...
				//...and the artist too
				newExif.setAttribute(ExifInterfaceExtended.TAG_EXIF_ARTIST,"Raaja Ar");
